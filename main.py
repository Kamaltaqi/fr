    ...
    ScreenManager:
        ...
        MDScreen:
            MDBoxLayout:
                ...
                MDTopAppBar:
                    title: app.reshape_text("تعرف على الفواكه")
                    elevation: 4
...
class MainApp(MDApp):
    ...
    def build(self):
        ...
        try:
            if platform == "android":
                self.request_permissions()  # طلب الأذونات عند بناء التطبيق على Android
            elif platform == "win":
                print("التطبيق يعمل على Windows. تجاوز طلب الأذونات.")
            self.load_model()
    ...
    def request_permissions(self):
        try:
            ...
            elif platform == "win":  # إذا كان النظام هو Windows
                print("التطبيق يعمل على Windows. لا حاجة لطلب الأذونات.")
        except Exception as e:
            print(f"خطأ في طلب الإذن: {str(e)}")
        ...
        try:
            if platform == "android":
                ...
                if not check_permission(Permission.CAMERA):
                    self.show_error_dialog("يرجى منح إذن الوصول إلى الكاميرا")
                    return
